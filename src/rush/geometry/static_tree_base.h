...
namespace rush
{
    ...
    class StaticTreeLeaf
    {
      ...
      public:
        ...
        size_t populate(TreeContent<Storage, Bounds>* pool, size_t from, size_t to);
        ...
        void intersections(const Collider& collider, std::unordered_set<TreeContent<Storage, Bounds>>& set,
    ...
    class StaticTreeNode
    {
        using ChildType = std::conditional_t<
            (Depth > 1), StaticTreeNode<Storage, Bounds, Dimensions, Type, MaxObjects, Depth == 1 ? Depth : Depth - 1>,
    ...
    class StaticTree
    {
      ...
      public:
        ...
        explicit StaticTree(const AABB<Dimensions, Type>& aabb, const Elements& elements);

        explicit StaticTree(const AABB<Dimensions, Type>& aabb, const std::vector<Bounds>& elements)
            requires std::is_void_v<Storage>;

        ~StaticTree();

        const StaticTreeNode<Storage, Bounds, Dimensions, Type, MaxObjects, Depth>& getRoot() const;

        template<typename Collider>
        void intersections(const Collider& collider, std::unordered_set<TreeContent<Storage, Bounds>>& set) const;
